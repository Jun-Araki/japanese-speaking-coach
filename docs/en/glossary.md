...
| Rater agreement | `rater_agreement` | Of 20 items rated by both the author and a second native speaker, the share receiving the same rating | **Will not be obtained (decided 2026-08-19). Kept in the table, marked not measured** (below) |
...
**`rater_agreement` will not be obtained (decided 2026-08-19).** The 20-item kit went out to the
first rater on 8/8 and nothing came back by the 8/16 deadline. The work was moved twice
afterwards — to the 13 September meetup, then forward to 24–30 August once it was clear the
rating needed no venue — and on **2026-08-19 it was dropped rather than moved a third time.**

**It was dropped because there was no credible route to a second rater, not because of the
calendar.** The conditions never changed: twenty items on paper, no public URL, no audio, any
native speaker of Japanese. Eleven days after the first request the kit had still not reached a
second person, which says the premise — that someone was available to ask — was the part that
was wrong.

**The kit had also gone stale.** It was built from the **baseline** (`baseline-v1`), not from the
implementation this repository publishes (`correction-v1`), so sending it as it stands would
return an agreement figure about a system nobody will use. Rebuilding it means re-rating the
author's own twenty items as well, about thirty minutes. **Committing certain rework for an
uncertain measurement is the wrong way round.**
...
**The cost is specific and is not being softened.** The private planning document says of
...
mitigation abandoned the weakness stands, so every place `correction_validity` is published must
say **that it is a single-rater figure with no independent check**. The number is not adjusted;
what it is a number *of* is stated accurately.

> **This is a weakness stated, not a weakness hidden.** Being able to write "one person scored
> this, and that person built it" is stronger than any wording that lets a reader assume an
> independent check happened. It is the same discipline as the 2026-08-16 correction, where a
> measurement that did not exist had been recorded as complete and was found and fixed here.
