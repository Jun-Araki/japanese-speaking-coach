...
# WHOSE VOICE IT IS, SAID BEFORE ANYONE WAITS FOR IT. Since 2026-08-24 the reply is
# read by the learner's own device rather than fetched from a provider, and the one
# thing that costs is that a phone with no Japanese voice installed says nothing at
# all. That silence is indistinguishable from a broken app unless it was mentioned
# first — and at a meetup, where phones differ and nobody can compare, it will happen
# to somebody. The text is always there, which is the half of this worth knowing.
VOICE_SOURCE_NOTE: Final = (
    "The reply is always written on screen. Your own phone or laptop reads it aloud, "
    "so a device with no Japanese voice installed will show the text without the sound."
)

# THE SAME PROMISE FOR THE OTHER PATH. `BROWSER_VOICE=0` sends for a recording
# instead, and that path goes quiet for a different reason: one free-tier key serves
# the whole room, so a minute's worth of requests is spent quickly. The learner does
# not need to know which of the two is running — only that silence is not breakage
# and the words are on the screen either way.
PROVIDER_VOICE_NOTE: Final = (
    "The reply is always written on screen. The voice is generated by an AI service "
    "shared by everyone using this demo, so it sometimes stays silent — the practice "
