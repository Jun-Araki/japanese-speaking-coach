...
BROWSER_VOICE = "1"           # 1 でブラウザ自身の音声（即座・無料）、0 で Gemini の音声（3.7秒）
...
- [ ] **返事が読み上げられるか。鳴らなくても画面には何も出ない**（2026-08-22 の決定）。
      — **既定はブラウザ自身の音声**（2026-08-24）。返事の下に「▶ もう一度」が出る。
        **端末に日本語の音声が無いと「この端末には日本語の音声が入っていません」と出る**——
        そのときは Secrets に `BROWSER_VOICE = "0"` で Gemini の音声に戻る（再ビルド不要）
      — **iPhone は要確認。** Safari は操作なしの再生を許さないので、
        自動では鳴らず「▶ もう一度」を押す必要があるかもしれない
      **`BROWSER_VOICE = "0"` のときに鳴らないなら Cloud のログを見る**——`[speak] no audio for ...` に理由が出る。
      `429 Too Many Requests` なら無料枠の速度上限で、**故障ではない**
      — **ログは静かなはず。** 2026-08-24 まで、起動のたびに `torchvision` が無いという
        traceback が数百行流れていた（Streamlit のファイル監視が `transformers` の
        遅延 import を踏むだけの無害なもの）。**`fileWatcherType = "none"` で消した**——
        探している 1 行が埋まっていては、ログを見ろと書いた意味が無い
