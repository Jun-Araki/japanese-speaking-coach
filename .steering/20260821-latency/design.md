...
> ### 2026-08-24 追記 — **案 B を既定にした。却下の前提が崩れた**
>
> 却下の理由は「無音になるかもしれない」だった。**その後に分かったのは、案 A のほうが
> 無音になるということである** — 無料枠の速度上限に1人が数ターンで当たり、
> 鳴るときも **3.7 秒**かかる。preview モデルなので消える可能性もある。
> 「たまに鳴らないかもしれない」は、もう差別化要因ではない。
>
> - **`BROWSER_VOICE=1` が既定。** 往復が無いので**ほぼ 0 秒**、レート上限も料金も無い
> - **`BROWSER_VOICE=0` で案 A に戻る**（環境変数1つ・再ビルド不要）
> - 端末に日本語の音声が無ければその旨を出す。**iPhone は操作の壁に当たる可能性があり、
>   だから「▶ もう一度」を必ず出す**（案 A の再生ボタンと同じ理屈）
> - **エコー捨ての秒数は推定になる。** ブラウザの音声は長さを教えてくれないので、
>   `speaking_seconds`（実測 7.3 文字/秒）で見積もる。`playback_seconds` は正確だが、
>   測るファイルがある案 A のときだけ使える
