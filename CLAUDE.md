...
- **スコープを勝手に広げない。** PLAN.md「**9/20 までにやらないこと**」に挙がっているもの
  （発音採点・ローカルモデル・React 画面・復習リストの画面・評価データ150件超・
  **DSPy / scikit-learn / AWS**）は提案も実装もしない
- **稼働は毎日2時間 ＝ 週14時間（平日2h・休日2h。2026-08-18 に週6.5h から戻した）。**
  **平日にも実装タスクを置いてよい**（旧「平日0.5h枠に実装を置かない」は撤回）。
  ただし **tasklist に積む見積りは週8〜10h まで**にする。**枠を100%埋めると必ず壊れる**というのが
  8/16 に学んだことで、残り5〜6hは緩衝として空けておく
- **期日は 9/20 に DoD、9/28 に応募開始。外部の固定日は 9/13（日）の Minna Shuugou だけ**
  （テスター手渡しと第二採点者の一致率を同時に取る、9月唯一の回）。
  **9/12 までに音声つきの URL が動いていること**が、この日程の生命線
