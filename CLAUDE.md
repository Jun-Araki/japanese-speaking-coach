...
| `functional-design.md` | 薄いまま | **Day 3（8/4）の訂正 JSON スキーマ確定後**。構成図・データ構造・エラーハンドリングを実物に合わせて増補する |
| `architecture.md` | 薄いまま | **Day 5（8/6）のベースライン測定後**。実測のモデル・レイテンシ・コストが数字で入ってから |
