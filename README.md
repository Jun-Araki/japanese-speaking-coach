...
**Every results table below is deliberately empty.** No number appears in this README until it
has been measured, and the method for measuring each one is already written down. Nothing here
is an estimate. What is built so far is listed under [Progress](#progress).
...
individually, which is what makes the set expandable rather than a one-off; ten were written by
...
- [ ] Baseline measured on the 40-item test split
